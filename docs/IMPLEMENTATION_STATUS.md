...
- 修正月末 NAV 資料點與月初 X 軸刻度不一致的問題；所有月度資料現在統一對齊所屬月份，5 月報告的最後標籤為 `2026-05`。
- 成立日與首筆 NAV 位於同一月份時不再重複繪製兩個資料點。
- 修正 Boya 報告的 Max Drawdown 欄位映射，改用動態 running-peak 最大回撤，不再誤用最差單月回報。
- 新增圖表月份對齊回歸測試，並完成輸出 PNG 視覺檢查。
...
- Boya 成立日的 `100.00` 基準 NAV 若同時以首筆月末紀錄存在，僅作為歷史與圖表基準，不再誤算成 `0.00%` 的月度回報；Boya 報告改顯示完整 Performance Matrix，月度樣本與波動率／年化／Sharpe 皆以實際回報月份計算。
