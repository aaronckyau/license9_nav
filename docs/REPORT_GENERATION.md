...
3. 重新計算 `legacy_excel_v1`，拒絕 duplicate、missing month、季報 quarter gap、invalid RFR 等問題。
...
- 使用者提供的 XSQ 2026 Q1 newsletter 是內建報告的章節與視覺依據；季報保留季度回報表，月報改為最近最多 12 個月的 NAV／月回報表，其餘章節順序一致。
...
- default/even/first footer 一致顯示 fund code、report ID、report period、generation time、page field；一般使用者看不到內部版本欄。
